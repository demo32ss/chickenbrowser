...
async function getPreviews(selectedFrame, scope, thunkArgs) {
  ...
  const allPreviews = [];
  const bindingReferences = await editor.getBindingReferences(selectedLocation, scope);
  (0, _context.validateSelectedFrame)(getState(), selectedFrame);

  for (const level in bindingReferences) {
    for (const name in bindingReferences[level]) {
      const previews = await generatePreviewsForBinding(bindingReferences[level][name], selectedLocation.line, name, client, selectedFrame.thread);
...
async function generatePreviewsForBinding(bindingData, pausedOnLine, name, client, thread) {
  if (!bindingData) {
    return [];
  ...
  for (let i = bindingData.refs.length - 1; i >= 0; i--) {
    const ref = bindingData.refs[i]; // Lines in CM6 is 1-based
    ...
    const {
      displayName,
      displayValue
    } = await getExpressionNameAndValue(name, bindingData.value, ref, client, thread); // Variable with same name exists, display value of current or
...
async function getExpressionNameAndValue(name, value, ref, client, thread) {
  ...
  while (meta) {
    ...
    if (!properties) {
      displayName += `.${meta.property}`; // Initially properties will be an array, after that it will be an object
